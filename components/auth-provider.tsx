...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Login function - stores credentials in AsyncStorage
  ...
  // Logout function - clears AsyncStorage and navigates to login
  const logout = async () => {
    try {
      await Promise.all([
      ...
      // Note: Cannot navigate to /login - screen doesn't exist yet
    } catch (error) {
  ...
  // Session expiry check - validate token every 60 seconds
  useEffect(() => {
