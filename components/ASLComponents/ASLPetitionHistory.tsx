...
export function ASLPetitionHistory({
  peticiones,
  onCancelar,
  onRate,
  ...
  const iconosEspecificos: {
    ...
    food: { icon: "flatware", iconType: "material", color: "#FF6B6B" },
    amenities: { icon: "sanitizer", iconType: "material", color: "#4ECDC4" },
    sanitizer: { icon: "sanitizer", iconType: "material", color: "#4ECDC4" },
    linens: { icon: "bed", iconType: "material", color: "#95E1D3" },
    linen: { icon: "bed", iconType: "material", color: "#95E1D3" },
    bed: { icon: "bed", iconType: "material", color: "#95E1D3" },
    comfort: {
    ...
    extra: { icon: "question-mark", iconType: "material", color: "#FDCB6E" },
    "air conditioning": {
      icon: "ac-unit",
      iconType: "material",
      color: "#2196F3",
    },
    "ac unit": { icon: "ac-unit", iconType: "material", color: "#2196F3" },
  ...
  const peticionesOrdenadas = useMemo(() => {
    ...
    return [...peticiones].sort((a, b) => {
      // Primero: activas primero (pending, in-progress)
      const estadoA = ordenEstado[a.status as string] || 5;
      const estadoB = ordenEstado[b.status as string] || 5;
      if (estadoA !== estadoB) return estadoA - estadoB;
      // Segundo: dentro de cada grupo, más reciente primero
  ...
  const getNumeroPeticion = (peticion: any) => {
    ...
    const ordenadas = [...peticiones].sort((a, b) => new Date(b.timestamp || 0).getTime() - new Date(a.timestamp || 0).getTime());
  ...
  return (
    <>
      ...
      <View style={styles.container}>
        {peticionesOrdenadas.map((peticion: any) => {
          ...
          return (
            <View key={peticion.id} style={styles.cardWrapper}>
              <View
                style={[styles.statusBar, { backgroundColor: config.color }]}
              />
              <View
                ...
                <View style={styles.mainRow}>
                  ...
                  <View
                    style={[
                      ...
                      { backgroundColor: `${tipoInfo.color}22` },
                    ]}
                  >
                      ...
                      <View style={styles.ratingDisplay}>
                        {[1, 2, 3, 4, 5].map((star) => (
                          <MaterialIcons
                            ...
                            size={16}
                            color={
                              star <= peticion.rating ? "#FFD700" : mutedColor
                            }
                          />
                        ))}
                      </View>
